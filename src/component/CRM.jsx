...
const CustomerCard = ({ customer, onClick }) => (
  <div 
    ...
    className="bg-[#181b21] hover:bg-[#1f232b] p-5 rounded-xl border border-[#2a2f38] cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-cyan-900/10 group animate-in fade-in zoom-in-95 flex flex-col gap-4"
  >
...
export default function CarDealershipCRM({ onLogout, onNavigate }) {
  ...
  // Dashboard Stats
  const stats = [
    { title: 'Total Leads', value: '1,248', trend: 12, icon: User, color: '#00D9FF' },
    { title: 'Cars Sold', value: '86', trend: 4.5, icon: Car, color: '#10b981' },
    { title: 'Revenue', value: '$2.4M', trend: -2.3, icon: TrendingUp, color: '#f59e0b' },
  ];

  const handleAction = (action, customer) => {
    console.log(`Action: ${action} for ${customer.name}`);
    // Implement specific action logic (open chat modal, etc.)
    if (action === 'loss_analysis' && onNavigate) {
